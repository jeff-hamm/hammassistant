...
class TileAuthenticator:
    ...
    def _calculate_checksum(self, data: bytes) -> int:
        ...
        for b in data:
            ...
            b2 = 0
            for i in range(8):
                b2 = ((b2 ^ b) & 1) if ((b2 >> 1) ^ 33800) else (b2 >> 1)
                if (b2 ^ b) & 1:
                    b2 = (b2 >> 1) ^ 33800
                else:
                    b2 = b2 >> 1
    ...
    async def program_song(self, song_data: bytes) -> bool:
        ...
        3. Send song data in blocks with prefix 5
        4. Each block includes 2-byte checksum
        """
        if not self._authenticated:
        ...
        try:
            ...
            # Format: [1 (command), length_low, length_high]
            length_bytes = struct.pack("<H", len(song_data))
            ready_payload = bytes([1]) + length_bytes
            
            prefix, response = await self.send_packets_async(
                5,  # Song prefix
                bytes([SongCommand.PROGRAM_READY]) + ready_payload,
                7,  # Song response
                timeout=5.0
            )
            ...
            # Step 2: Send song data in blocks
            max_payload = self._toa_processor.max_payload_size - 1
            ...
            for block_num in range(num_blocks):
                ...
                block_data = song_data[fw_index:fw_index + num_bytes_this_block]
                
                # Calculate checksum for this block
                checksum = self._calculate_checksum(block_data)
                block_with_checksum = block_data + struct.pack("<H", checksum)
                
                # Send block data in packets
                max_packet_len = min(len(block_with_checksum), max_payload)
                
                for pkt_start in range(0, len(block_with_checksum), max_packet_len):
                    pkt_end = min(pkt_start + max_packet_len, len(block_with_checksum))
                    pkt_data = block_with_checksum[pkt_start:pkt_end]
                    
                    is_last_packet = (pkt_end >= len(block_with_checksum))
                    ...
                    if is_last_packet:
                        ...
                        if len(resp) > 0 and resp[0] == 32:  # Error
                            _LOGGER.error("Program block error: %s", resp.hex())
                            return False
                    else:
            ...
            _LOGGER.error("Song programming error: %s", e)
            return False
